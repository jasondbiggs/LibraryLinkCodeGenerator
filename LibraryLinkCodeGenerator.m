...
WriteLibrarySignatures[sourceFiles_, destinationFile_] := Module[
	{signatures = scanForSignatures /@ sourceFiles, outstring, symbols, filestring},
	If[!MatchQ[signatures, {{_,_}..}], throw[signatures,"no signatures found"]];
	symbols = scanForSymbols @ Flatten[DeleteCases[signatures[[All, 2]],_Enumerate, Infinity]][[All, 1]];
	outstring = UsingFrontEnd @ StringRiffle[
		fileDefinitionString @@@ signatures,
		"\n"
	];
	If[!StringQ[outstring],throw[$Failed,"bad signature conversion"]];
	outstring = StringJoin[
		$beginCodeBlock,
		"\n\n",
		commentTemplate["Exported Symbols"],
		"\n\n",
		StringRiffle[
			symbols,
			"\n"
	...
	If[FileExistsQ[destinationFile],
		filestring = Import[destinationFile, "Text"];
		outstring = StringReplace[filestring,
...
scanForSymbols[input_] := DeleteDuplicates @ Replace[input,
	{
		{sym_} :> sym,
		{obj_, method_} :> obj,
		{obj_, sub_, method_} :> sub
	},
	{1}
]
